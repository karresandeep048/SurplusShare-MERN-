...
const OrderTracker = () => {
    ...
    useEffect(() => {
        ...
        const simulationDurationMs = Math.min(600000, Math.max(30000, estimatedTripMins * 3000));
        ...
        const timer = setInterval(() => {
            ...
            if (newProgress >= 1) {
                // Only mark as arrived — do NOT auto-notify the donor
                // Receiver must manually click "I Have Arrived" button
                setArrived(true);
                clearInterval(timer);
            }
        }, stepInterval);
                        ...
                        {isSupplier ? (
                            ...
                            <div className="space-y-5">
                                ...
                                {donorNotified ? (
                                    ...
                                    <div className="text-center py-5 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 rounded-3xl border-2 border-amber-300 p-5 shadow-sm animate-fade-in">
                                        <div className="w-14 h-14 bg-amber-100 text-amber-700 rounded-2xl flex items-center justify-center mx-auto mb-3 animate-bounce">
                                            <Bell className="w-7 h-7" />
                                        </div>
                                        <h3 className="text-lg font-black text-slate-900 mb-1">You've Reached the Venue!</h3>
                                        <p className="text-xs text-slate-600 font-medium mb-4 max-w-xs mx-auto">
                                            Tap the button below to notify <strong>{reservationData?.foodListing?.supplier?.name || 'the food donor'}</strong> that you have arrived and are ready to collect the food.
                                        </p>
                                        <button
                                            onClick={handleNotifyArrival}
                                            disabled={notifying}
                                            className="w-full bg-gradient-to-r from-brand-600 to-emerald-600 hover:from-brand-700 hover:to-emerald-700 text-white text-sm font-black px-6 py-4 rounded-2xl shadow-lg shadow-brand-500/30 transition-all inline-flex items-center justify-center gap-2 animate-pulse"
                                        >
                                            {notifying ? <Loader2 className="w-5 h-5 animate-spin" /> : <Bell className="w-5 h-5" />}
                                            <span>I Have Arrived — Notify Food Donor</span>
                                        </button>
                                        <p className="text-[11px] text-amber-700 font-semibold mt-3">
                                            The donor will receive an email + dashboard alert with your pickup code.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        ...
                                        <button
                                            onClick={handleNotifyArrival}
                                            disabled={notifying}
                                            className="w-full bg-gradient-to-r from-brand-600 to-emerald-600 hover:from-brand-700 hover:to-emerald-700 text-white text-xs font-bold px-4 py-3.5 rounded-2xl shadow-md shadow-brand-500/20 transition-all flex items-center justify-center gap-2"
                                        >
                                            {notifying ? <Loader2 className="w-4 h-4 animate-spin" /> : <Bell className="w-4 h-4" />}
                                            <span>I Have Arrived at the Venue</span>
                                        </button>
                                    </div>
                                )}
                            </div>
